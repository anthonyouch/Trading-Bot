...
class Strategy:
    ...
    def _check_tp_sl(self, trade: Trade):
        ...
        if trade.side == "long":
            ...
            if self.stop_loss is not None:
                if price <= trade.entry_price * (1 - self.stop_loss / 100):
                    print("loss entry price: " + str(trade.entry_price))
                    sl_triggered = True
            if self.take_profit is not None:
                if price >= trade.entry_price * (1 + self.take_profit / 100):
                    print("profit entry price: " + str(trade.entry_price))
                    tp_triggered = True
        ...
        if tp_triggered or sl_triggered:
            ...
            #print("trade_quantity: " + str(round_decimals_down(trade.quantity * 0.999, 6)))

            #print("balance: " + str(self.client.get_balances()))

            if self.client.get_balances()[self.contract.symbol[:3]].free >= round_decimals_up(trade.quantity * 0.999, 6):
                order_status = self.client.place_order(self.contract, "MARKET",
                                                       round_decimals_up(trade.quantity * 0.999, 6), order_side)
            else:
                order_status = self.client.place_order(self.contract, "MARKET",
                                                       round_decimals_down(trade.quantity * 0.999, 6), order_side)
